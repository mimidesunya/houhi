/**
 * 一般文書の PDF / Word / ODT / PowerPoint ファイルをOCR・テキスト抽出し、Markdown化するプログラム。
 * AIプロバイダーとして Gemini / Claude / OpenAI を利用できます。
 * 一般文書向けのMarkdown整形ルールを使って、段落統合や見出し化を行います。
 *
 * 入力:
 * - `.pdf`
 * - `.docx`
 ...
 * - Office / ODT / PowerPoint は対応する Markdown を同じ場所に出力します。
 *
 * オプション:
 ...
 *
 * 補足:
 * - ディレクトリ指定時は `.pdf` / `.docx` / `.doc` / `.odt` / `.pptx` を走査します。
 * 
 * 使い方:
 *   node src/ocr_general_doc.js <入力ファイルパス または ディレクトリパス> [--batch_size <枚数>] [--start_page <開始ページ>] [--end_page <終了ページ>]
 */
const fs = require('fs');
...
async function main() {
    ...
    if (inputPaths.length === 0) {
        ...
        console.log(" 使い方: node ocr_general_doc.js <input_path...> [--batch_size <n>] [--ai gemini|claude|openai] [--mode batch|sync] [--ndlocr] [--ndlocr_only] [--prefer_pdf_text]");
        console.log("-------------------------------------------------------");
        return;
    }
    ...
    const processFile = async (filePath) => {
        ...
        if (ext === ".pdf") {
            ...
            await pdfToText(filePath, batchSize, startPage, endPage, GENERAL_DOC_STYLE, aiProvider, processMode, useNdlocr, ndlocrOnly, preferPdfText);
        } else if (ext === ".docx") {
            if (ndlocrOnly) {
